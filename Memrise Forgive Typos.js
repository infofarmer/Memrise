...
var onLoad = function($) {
	...
	r = f +a.map(function (v, i, a) { return codes[v] }).filter(function (v, i, a) {
	...
	var check_answer = function(input) {
		...
		if (category === 'Chinese') {
			...
			if (handleFrench(given, correct)) {
				...
				if (given.toString() == correct.toString()) 
				{
					return true;
				}
				
				//if (soundex(given.toString()) == soundex(correct.toString())) 
				//{
				//	alert(given.toString() + '  Correct: ' + correct.toString());
				//	return false; 
				//}
		...
		else if ( given.toString().length == 0 || (given.toString().length > 0 && correct.toString().substring(0,1) != given.toString().substring(0,1)))
		{
			alert(' Hint: ' + correct.toString().substring(0,1));
			return false;
		}
